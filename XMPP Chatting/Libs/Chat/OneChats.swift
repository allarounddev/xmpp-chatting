...
//  OneChat
//
//  Created by Victor Belenko on 22/2/2016.
...
public class OneChats: NSObject, NSFetchedResultsControllerDelegate {
	...
	private func getActiveUsersFromCoreDataStorage() -> NSArray? {
		...
		if let predicateString = LoginHandler.sharedInstance.userId {
			...
			do {
				...
				for message in results! {
					...
					if element.attributeStringValueForName("to") != LoginHandler.sharedInstance.userId! && !(element.attributeStringValueForName("to") as NSString).containsString(LoginHandler.sharedInstance.userId!) {
	...
	private func getUserFromXMPPCoreDataObject(jidStr jidStr: String) {
		...
		if OneChat.sharedInstance.xmppStream == nil {
			...
			predicate = NSPredicate(format: "jidStr == %@ AND streamBareJidStr == %@", jidStr, LoginHandler.sharedInstance.userId!)
		}
